...
class Data:
    def __init__(self, config):
        ...
        # 测试集中前几天的数据会被删掉，因为不够一个time_step
    ...
    def get_train_and_valid_data(self):
        ...
        if not self.config.do_continue_train:
            ...
            # 比如：1-20行，21-40行……到数据末尾，然后又是2-21行，22-41行……到数据末尾……
            # 这样才可以把上一个样本的final_state作为下一个样本的init_state，而且不能shuffle
            train_x = [
                feature_data[
                    start_index + i * self.config.time_step:
                    start_index + (i+1) * self.config.time_step
                ]
        ...
        train_x, valid_x, train_y, valid_y = train_test_split(
            train_x, train_y, test_size=self.config.valid_data_rate,
    ...
    def get_test_data(self, return_label_data=False):
        ...
        test_x = [
            feature_data[
                self.start_num_in_test+i*sample_interval:
                self.start_num_in_test+(i+1)*sample_interval
